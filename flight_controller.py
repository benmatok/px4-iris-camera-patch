...
class DPCFlightController:
    ...
    def compute_action(self, state_obs, target_cmd, tracking_uv=None, tracking_size=None, extra_yaw_rate=0.0, foe_uv=None, velocity_est=None, position_est=None, velocity_reliable=False):
        ...
        if tracking_uv:
            ...
            # Check if external FOE is provided
            if foe_uv:
                 calc_foe_px = foe_uv
            elif velocity_est:
            ...
            # Convert Normalized FOE to Pixels for Error Calculation
            calc_foe_px = None
            if calc_foe_norm:
            ...
            if calc_foe_px:
                 ...
                 # Normalize Error for Gain stability (pixels -> normalized)
                 # yaw_rate = -k * (u_err_norm)
                 u_err_norm = u_err / self.cam_fx
                 yaw_rate_cmd = -ctrl.k_yaw * u_err_norm
            else:
                 # Fallback: Center Target in Image
            ...
            if not self.dive_initiated:
                ...
                if calc_foe_px:
                    ...
                    # Target below FOE (+v_err) -> Pitch Down (Neg Rate Cmd -> Sim Nose Down)
                ...
                # Pitch Rate Clamp (Normal Flight)
        ...
        # --- Active Braking / Hard Limit Logic ---
        # Overrides visual guidance if speed is dangerous
        if velocity_est:
             limit = 6.0

             # VIO Consistency Check
             vio_vz_enu = -velocity_est['vz'] # NED to ENU
             est_vz_error = abs(vio_vz_enu - self.est_vz)

             # Only brake if VIO is plausible AND Speed is high
             # And ignore if VIO is crazy high (> 10.0)
             ...
             if vel_mag > limit:
                  if vio_plausible:
                      ...
                      target_pitch_brake = min(0.8, target_pitch_brake)

                      k_p_brake = 2.0
                      pitch_error = target_pitch_brake - pitch
                      pitch_brake_rate = k_p_brake * pitch_error

                      # Tighter Clamp Rate for smoother braking
                      pitch_brake_rate = max(-0.5, min(0.5, pitch_brake_rate))
                      ...
                      logger.warning(f"VIO High Velocity ({vel_mag:.2f}) but Implausible (Vz err={est_vz_error:.1f}). Ignoring Brake.")

        # --- Safety Floor / Ground Collision Avoidance ---
        if obs_pz is not None and obs_pz < 15.0 and self.est_vz < -3.0:
             logger.debug(f"SAFETY FLOOR: pz={obs_pz:.1f}, vz={self.est_vz:.1f}")
