...
class ControlConfig:
    ...
    v_target_intercept: float = 0.27
    v_target_pitch_threshold: float = -1.2

    # Thrust
    thrust_base_intercept: float = 0.6
    thrust_base_slope: float = 0.3
    thrust_min: float = 0.15
    thrust_max: float = 0.5 # Base max

    rer_target: float = 0.35
    k_rer: float = 2.0

    flare_gain: float = 1.2
    flare_threshold_offset: float = 0.25

    # Final Mode
    final_mode_v_threshold_low: float = -0.1
    final_mode_v_threshold_high: float = 0.4
    ...
    hover_pitch_threshold: float = -0.9 # Pitch < -0.9 is Steep
    hover_yaw_threshold: float = 0.1 # Align threshold

    # Phase 2: Attack
    attack_pitch_setpoint: float = 0.15 # Fixed screen position lock

    # Brake / Thrust
    thrust_cruise: float = 0.65
    brake_k_rer: float = 0.8
    brake_rer_threshold: float = 0.1
